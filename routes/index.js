...
indexRouter.get('/', function(req, res, next) {
  ...
  mySqlConnection.query(request, [1, 2, 3], (err, rows, fields) => {
    ...
    if (rows[0].length === 0) {
      ...
      res.render('index', { title: 'LibExpress', subTitle: 'Listes Articles', rows: articles, page: page , previousPage: previousPage , nextPage: nextPage, count: count, limit: limit,categories: categories});
    }
  });
});
...
indexRouter.get('/:category', function(req, res, next) {
  console.log('filtre');
  ...
  const nextPage = page + 1;
  const previousPage = page - 1;
  const request = `SELECT article.* from article left outer join r_art_cat AS rac ON  rac.id_article = article.id where (rac.id_category is null or rac.id_category =${id}) limit ${limit} offset ${offset};
  SELECT COUNT(article.id) as count from article left outer join r_art_cat AS rac ON  rac.id_article = article.id where (rac.id_category is null or rac.id_category =${id});
  SELECT * from category; `
  mySqlConnection.query(request, [1, 2, 3], (err, rows, fields) => {
    if (err) throw err;
    if (rows[0].length === 0) {
